...
class Client extends EventEmitter {
    ...
    handleRequest(req, res) {
        ...
        // Prepare headers with X-Forwarded-* headers for proxy transparency
        const headers = { ...req.headers };

        // Debug: Log incoming headers
        this.debug('Incoming X-Forwarded-Proto: %s', headers['x-forwarded-proto']);
        this.debug('Secure flag: %s', this.secure);

        // Set X-Forwarded-Proto if not already set by upstream proxy (like Caddy)
        // This tells the local app whether the original request was HTTPS or HTTP
        if (!headers['x-forwarded-proto']) {
            headers['X-Forwarded-Proto'] = this.secure ? 'https' : 'http';
            this.debug('Setting X-Forwarded-Proto to: %s', headers['X-Forwarded-Proto']);
        }
        ...
        this.debug('Headers to local app: %O', headers);
