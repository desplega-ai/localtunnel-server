...
// Timeout for idle tunnel connections
// Increased from 60s to 10 minutes to support WebSocket and long-lived HTTP tunnels
// Can be overridden via SOCKET_TIMEOUT environment variable (in seconds)
const DEFAULT_SOCKET_TIMEOUT = (process.env.SOCKET_TIMEOUT ? parseInt(process.env.SOCKET_TIMEOUT) * 1000 : 10 * 60 * 1000);
...
class TunnelAgent extends Agent {
    ...
    _onConnection(socket) {
        this.debug(`new tcp socket -> setting timeout to ${this._socketTimeout}ms`);
